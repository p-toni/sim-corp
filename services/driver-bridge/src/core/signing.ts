...
export function createEnvelopeSigner(options: SignerOptions): EnvelopeSigner {
  ...
  if ((!kid || !privateKeyB64) && allowAutoGenerate) {
    ...
    options.logger?.info?.("driver-bridge: generated signing key", { kid, publicKeyB64 });
  }
    ...
    try {
      const response = await fetch(`${kernelUrl.replace(/\/$/, "")}/devices`, {
        ...
        body: JSON.stringify({
          ...
          orgId: options.orgId,
          publicKeyB64,
          meta: { source: "driver-bridge" }
        })
      });
      if (!response.ok && response.status !== 409) {
        options.logger?.warn?.("driver-bridge: failed to register device key", response.status);
        return;
      }
      registeredKids.add(kid);
    } catch (error) {
      options.logger?.warn?.("driver-bridge: device key registration error", error);
    }
  };
