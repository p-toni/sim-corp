...
export interface EnvelopeSigner {
  signEnvelope(envelope: TelemetryEnvelope): TelemetryEnvelope;
...
export function createEnvelopeSigner(options: SignerOptions): EnvelopeSigner {
  ...
  if ((!kid || !privateKeyB64) && allowAutoGenerate) {
    ...
    options.logger?.info?.("sim-publisher: generated signing key", { kid, publicKeyB64 });
  }
  ...
  const ensureRegistered = async (): Promise<void> => {
    if (!autoRegister || !kernelUrl || registeredKids.has(kid)) {
      return;
    }
    try {
      const response = await fetch(`${kernelUrl.replace(/\/$/, "")}/devices`, {
        ...
        body: JSON.stringify({
          ...
          orgId: options.orgId,
          publicKeyB64,
          meta: { source: "sim-publisher" }
        })
      });
      if (!response.ok && response.status !== 409) {
        options.logger?.warn?.("sim-publisher: failed to register device key", response.status);
        return;
      }
      registeredKids.add(kid);
    } catch (error) {
      options.logger?.warn?.("sim-publisher: device key registration error", error);
    }
  };

  const signEnvelope = (envelope: TelemetryEnvelope): TelemetryEnvelope => {
    const payload = { ...envelope, kid };
