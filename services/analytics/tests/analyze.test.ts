...
describe("analyzeSession", () => {
  ...
  it("applies overrides to markers and deltas", () => {
    const telemetry = [
      { ts: new Date(0).toISOString(), machineId: "m", elapsedSeconds: 0, btC: 120, rorCPerMin: 0 },
      { ts: new Date(300_000).toISOString(), machineId: "m", elapsedSeconds: 300, btC: 198, rorCPerMin: 10 },
      { ts: new Date(600_000).toISOString(), machineId: "m", elapsedSeconds: 600, btC: 210, rorCPerMin: 8 }
    ...
    const analysis = analyzeSession({
      ...
      telemetry,
      events,
      overrides: [{ eventType: "FC", elapsedSeconds: 330, updatedAt: new Date().toISOString(), source: "HUMAN" }]
    });

    expect(analysis.fcSeconds).toBe(330);
    expect(analysis.overrideDeltasSeconds.FC).toBe(30);
    expect(analysis.eventTimeSource.FC).toBe("OVERRIDDEN");
  });
});
